...
/*
  ...
  Double_t xBj, Q2, mesonmomentum, perweight;
  Int_t prod,evtMode;
  ...
  Double_t hh_lower_bound[] = {0,0,0.000001}; //lower bound of 1d, 2dx, 2dy
  Double_t hh_upper_bound[] = {2,2,10}; //upper bound of 1d, 2dx, 2dy
  ...
  TH2D *h_weight_var = new TH2D("h_weight_var","distribution of weight all mode included",80,hh_lower_bound[1],hh_upper_bound[1],100,hh_lower_bound[2],hh_upper_bound[2]);
  BinLogY(h_weight_var);

  TH2D *hh2d[] = {hxQ2_QE_2d, hxQ2_RES_2d, hxQ2_DIS_2d, hxQ2_2p2h_2d, h_weight_var};
  ...
  for (Int_t i=0; i<nentries; i++)
  {
    ...
    else
    {
      ...
      {
        kk = 0;
        hh2d[kk]->Fill(xBj,perweight);
        hh1d[kk]->Fill(xBj,perweight);        
        h_weight_var->Fill(xBj,perweight);
      }
