...
void fill_hist(TString fn, int anaid, TString Emode, double &histnormFactor
  , TH2D* (&hist_2d_lst)[2], TH1D* (&hist_1d_lst)[3],TH1D* (&hist_1d_mom_lst)[12])
{
  ...
  cout<<"events for energy "<<Emode<<" : "<<nentries<<endl;
  int npi_err{0};
  for (Int_t i=0; i<nentries; i++)
  {
    ...
    hist_2d_lst[0]->Fill(xBj,Q2,perweight);
    hist_2d_lst[1]->Fill(xBj,Wtrue,perweight);
    ...
    {
      hist_1d_lst[ii]->Fill(hh1d_var_lst[ii],perweight);
    }

    for(Int_t ii=0; ii<12; ii++)
    {
      hist_1d_lst[ii]->Fill(hh1d_mom_var_lst[ii],perweight);
    }
...
void hist_nuSTORM_musig_GiBUU_2()
{
  ...
  */
  ...
  Double_t hh1d_hist_lower_bound[] = {0,0,0.8};
  Double_t hh1d_hist_upper_bound[] = {1,2.8,2.8};
  ...
  TString temp_name = "";
  TString temp_tit = "";
  TH2D *hh2d[n_E][2]; //list of 2d hist {hxQ2_E7, hxW_E7}
  TString hh2d_name_lst[n_E][2];
  TString hh2d_tit_lst[n_E][2];
  ...
  TH1D *hh1d_mom[n_E][12];//={hxQ2_E7_scattertheta, hxQ2_E7_scattermomentum,  hxQ2_E7_mesontheta, hxQ2_E7_mesonmomentum, 
  ...
  TString hh1d_mom_name_lst[n_E][12];
  ...
  for(int ii=0; ii<nh2d; ii++){
    ...
    if(normal_choice==0)
    {
      hh2d[ii/2][ii%2]->Scale(1/histnormFactor);
      remove_neg(hh2d[ii/2][ii%2]);
    }
    else if(normal_choice==1)
    {
      col_normal(hh2d[ii/2][ii%2]);
    }
    hh2d[ii/2][ii%2]->GetXaxis()->SetTitle("x_{Bj}");
    if(ii%2==0)
    {
      hh2d[ii/2][ii%2]->GetYaxis()->SetTitle("Q^{2}(GeV^{2})");
      gPad->SetLogy(1); //set y axis to log-1 lin-0 scale
      gPad->Update();
    }
    else if(ii%2==1)
    {
      hh2d[ii/2][ii%2]->GetYaxis()->SetTitle("W(GeV)");
      gPad->SetLogy(0); //set y axis to log-1 lin-0 scale
      gPad->Update();
    }
    hh2d[ii/2][ii%2]->SetContour(1000);
    //hh2d[ii]->Draw("TEXT");
    hh2d[ii/2][ii%2]->Draw("colz");

    gStyle->SetPadRightMargin(0.2);
    //redraw legend and update bottom margin dependent on number of entries

    // add this plot to plotbook
    c1->Print("plot.pdf",hh2d[ii/2][ii%2]->GetTitle());
    c1->Print(Form("png/%s_%s.png", anatag, hh2d[ii/2][ii%2]->GetTitle()));
  ...
  */
  ...
  for(int ii=0; ii<nh1d_mom; ii++)
  {
    ...
    diff_xsec(hh1d_mom[ii/12][ii%12], histnormFactor);
    hh1d_mom[ii/12][ii%12]->GetYaxis()->SetTitle("differential cross section(cm^{2}/GeV/nucleon)");
    
    //leg->SetFillStyle(0);
    hh1d_mom[ii/12][ii%12]->SetLineStyle(kSolid);
    hh1d_mom[ii/12][ii%12]->SetLineColor(cols[ii/count_num]);
    hh1d_mom[ii/12][ii%12]->SetFillStyle(3001);
    ...
    switch (ii%count_num)
    {
      ...
      case 0:
        stk_mu_theta.Add(hh1d_mom[ii/12][ii%12]);
        leg_mu_theta.AddEntry(hh1d_mom[ii/12][ii%12],hh1d_mom[ii/12][ii%12]->GetTitle(),"fl");
        break;

      case 1:
        stk_mu_mom.Add(hh1d_mom[ii/12][ii%12]);
        leg_mu_mom.AddEntry(hh1d_mom[ii/12][ii%12],hh1d_mom[ii/12][ii%12]->GetTitle(),"fl");
        break;

      case 2:
        stk_pi_theta.Add(hh1d_mom[ii/12][ii%12]);
        leg_pi_theta.AddEntry(hh1d_mom[ii/12][ii%12],hh1d_mom[ii/12][ii%12]->GetTitle(),"fl");
        break;
    
      case 3:
        stk_pi_mom.Add(hh1d_mom[ii/12][ii%12]);
        leg_pi_mom.AddEntry(hh1d_mom[ii/12][ii%12],hh1d_mom[ii/12][ii%12]->GetTitle(),"fl");
        break;
    
      case 4:
        stk_recoil_theta.Add(hh1d_mom[ii/12][ii%12]);
        leg_recoil_theta.AddEntry(hh1d_mom[ii/12][ii%12],hh1d_mom[ii/12][ii%12]->GetTitle(),"fl");
        break;
    
      case 5:
        stk_recoil_mom.Add(hh1d_mom[ii/12][ii%12]);
        leg_recoil_mom.AddEntry(hh1d_mom[ii/12][ii%12],hh1d_mom[ii/12][ii%12]->GetTitle(),"fl");
        break;
    
      case 6:
        stk_baryon_theta.Add(hh1d_mom[ii/12][ii%12]);
        leg_baryon_theta.AddEntry(hh1d_mom[ii/12][ii%12],hh1d_mom[ii/12][ii%12]->GetTitle(),"fl");
        break;
    
      case 7:
        stk_baryon_mom.Add(hh1d_mom[ii/12][ii%12]);
        leg_baryon_mom.AddEntry(hh1d_mom[ii/12][ii%12],hh1d_mom[ii/12][ii%12]->GetTitle(),"fl");
        break;
    
      case 8:
        stk_neutron_mom.Add(hh1d_mom[ii/12][ii%12]);
        leg_neutron_mom.AddEntry(hh1d_mom[ii/12][ii%12],hh1d_mom[ii/12][ii%12]->GetTitle(),"fl");
        break;
    
      case 9:
        stk_dpt.Add(hh1d_mom[ii/12][ii%12]);
        leg_dpt.AddEntry(hh1d_mom[ii/12][ii%12],hh1d_mom[ii/12][ii%12]->GetTitle(),"fl");
        break;
    
      case 10:
        stk_dphit.Add(hh1d_mom[ii/12][ii%12]);
        leg_dphit.AddEntry(hh1d_mom[ii/12][ii%12],hh1d_mom[ii/12][ii%12]->GetTitle(),"fl");
        break;
    
      case 11:
        stk_dalphat.Add(hh1d_mom[ii/12][ii%12]);
        leg_dalphat.AddEntry(hh1d_mom[ii/12][ii%12],hh1d_mom[ii/12][ii%12]->GetTitle(),"fl");
        break;
...
/*
  THStack stk_lst[] = {stk_x, stk_Q2, stk_W, stk_mu_theta, stk_mu_mom, stk_pi_theta, stk_pi_mom, stk_recoil_theta, stk_recoil_mom
    , stk_baryon_theta, stk_baryon_mom, stk_neutron_mom, stk_dpt, stk_dphit, stk_dalphat};

  TLegend leg_lst[] = {leg_x, leg_Q2, leg_W, leg_mu_theta, leg_mu_mom, leg_pi_theta, leg_pi_mom, leg_recoil_theta, leg_recoil_mom
    , leg_baryon_theta, leg_baryon_mom, leg_neutron_mom, leg_dpt, leg_dphit, leg_dalphat};

  string tit_lst[15][5] = {{"x_{Bj}", "#frac{d#sigma}{dx_{Bj}} (cm^{2}/x_{Bj}/nucleon)", "x_{Bj} of diff E modes", "x_{Bj} of diff E modes", "x_{Bj} of diff E modes"},
  ...
  for(int ii; ii<n_stk; ++ii)
  {
    ...
    gStyle->SetPadLeftMargin(0.1);
    leg_lst[ii].Draw();
    c1->Print("plot.pdf",tit_lst[ii][3].c_str());
...
*/
  ...
  //delete leg_neutron_mom;
  
  stk_dpt.Draw(opt_nostack);
  stk_dpt.GetXaxis()->SetTitle("dp_{t}(GeV)");
  stk_dpt.GetYaxis()->SetTitle("#frac{d#sigma}{dp_{t}}(cm^{2}/GeV/nucleon)");
  ...
  stk_dphit.GetXaxis()->SetTitle("d#phi_{t}(degree)");
  stk_dphit.GetYaxis()->SetTitle("#frac{d#sigma}{d#phi_{t}}(cm^{2}/GeV/nucleon)");
  ...
  stk_dalphat.GetXaxis()->SetTitle("d#alpha_{t}(degree)");
  stk_dalphat.GetYaxis()->SetTitle("#frac{d#sigma}{d#alpha_{t}}(cm^{2}/GeV/nucleon)");
